...
const GenerateMarketingPlanOutputSchema = z.object({
  weeklyPlan: z.array(DailyTaskSchema).describe('A 7-day marketing plan with daily themes and tasks.'),
});
...
Tu tarea es crear un plan de marketing de contenido semanal (Lunes a Domingo) para una empresa con la siguiente descripción:
{{{businessDescription}}}

El enfoque principal debe estar en la plataforma: {{{socialMediaFocus}}}. También debes incluir ideas para Facebook e Instagram.

Para cada día de la semana, define un tema central y una lista de 2 a 3 tareas específicas y accionables. Las tareas deben ser prácticas y diseñadas para generar interés, educar a la audiencia y capturar leads.
...
Genera un plan completo para toda la semana.
`,
});
